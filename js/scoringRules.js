function calculateSum(dice, target) {
    return dice.filter(num => num === target).reduce((sum, num) => sum + num, 0);
}
...
function threeOfAKind(dice) {
    const counts = Array.from({ length: 6 }, (_, num) => dice.filter(die => die === num + 1).length);
    return counts.some(count => count >= 3) ? dice.reduce((sum, num) => sum + num, 0) : 0;
}

function fourOfAKind(dice) {
    const counts = Array.from({ length: 6 }, (_, num) => dice.filter(die => die === num + 1).length);
    console.log(counts);
    return counts.some(count => count >= 4) ? dice.reduce((sum, num) => sum + num, 0) : 0;
}
...
function upperTableTotal() {
    let sum = 0;
    for (let i=0; i <= 5; i++){
        sum += Number(document.getElementById(`${functionNames[i].name.toLowerCase()}-${currentPlayer}`).textContent);
    }
    return sum;
}
